...
const nextQuestion = async () => {
  ...
  const timeSpent = Math.floor((Date.now() - questionStartTime) / 1000);
    // 2️⃣ Always update questionTimes
  setQuestionTimes(prev => {
    const updated = [...prev, timeSpent];
    console.log("✅ Updated Question Times:", updated);
    return updated;
  });
  ...
  if (currentQuestion < questions.length - 1) {
    ...
    try {
      ...
      const submission = {
        studentId: challengeData.studentId,
        challengeId: challengeData.challengeId,
        answers: questions.map((q, index) => ({
          questionId: q.id,
          attemptStatus: answers[index] === "SKIPPED" ? "Skipped" : "Answered",
          selectedAnswer: answers[index] === "SKIPPED" ? null : answers[index],
          answerTimeInSeconds: questionTimes[index] || 0
        })),
        startTime: new Date(challengeStartTime).toISOString(),
        endTime: new Date().toISOString(),
        totalTimeForChallenge: challengeTime,
        ...
        submissionStatus: (() => {
          const attempted = Object.values(answers).filter(a => a !== "SKIPPED").length;
          if (attempted === 0) return "skipped";
          if (attempted === questions.length) return "completed";
          return "partially_completed";
        })(),
        metrics: {
          totalQuestions: questions.length,
          attempted: Object.values(answers).filter(a => a !== "SKIPPED").length,
          skipped: Object.values(answers).filter(a => a === "SKIPPED").length
        },
        score: null,
        rank: null,
        processedAt: null
      ...
      const docRef = doc(
        firestore, 
        "MoAChallenges/DWMSChallenges/Challenges", 
        challengeData.challengeId,
        "submissions",
        `${challengeData.studentId}_${Date.now()}`
      );
        ...
        <div className="bg-white/10 backdrop-blur-xl border border-white/20 rounded-3xl p-8 shadow-2xl max-w-md w-full">
          <div className="text-center space-y-6">
            ...
            <p className="text-white/80">
              We couldn't load this challenge's data. Please try another one.
            </p>

            <button
              onClick={() => navigate("/challenges")}
              className="bg-gradient-to-r from-purple-500 to-pink-500 hover:from-purple-600 hover:to-pink-600 text-white font-bold py-3 px-6 rounded-2xl transition-all duration-300 transform hover:scale-105 shadow-lg shadow-purple-500/25 w-full"
            >
              Browse Challenges
            </button>
          </div>
          ...
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center space-x-4">
              ...
              <div>
                ...
                <p className="text-white/80">{challengeData.type}</p>
              </div>
            </div>
